...
import { Menu, X, ChevronRight, Code, FileText, Hash, ChevronLeft, ChevronRight as ChevronRightIcon } from 'lucide-react'; // Renamed ChevronRight for clarity

const LeetCodeClone = () => {
    const [questions, setQuestions] = useState([]);
    const [currentQuestion, setCurrentQuestion] = useState(null);
    const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0); // New state for index
    ...
    // Load first question when questions are fetched or when currentQuestionIndex changes
    useEffect(() => {
        if (questions.length > 0) {
            loadQuestion(questions[currentQuestionIndex].folder);
        }
    }, [questions, currentQuestionIndex]); // Depend on questions and currentQuestionIndex
        ...
        <div className="min-h-screen bg-gray-900 relative overflow-hidden">
            ...
            <header className="relative z-50 p-4">
                <div className="flex items-center justify-between">
                    ...
                    {currentQuestion && (
                        <div className="flex items-center space-x-3"> {/* Container for navigation and question number */}
                            <button
                    ...
                    <div className="space-y-3 max-h-[calc(100vh-200px)] overflow-y-auto custom-scrollbar">
                        {questions.map((question, index) => (
                            <button
                                key={question.folder}
                                onClick={() => {
                                    setCurrentQuestionIndex(index); // Set index when selecting from menu
                                    loadQuestion(question.folder);
                                }}
